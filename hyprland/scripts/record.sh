...
if pgrep -f "wl-screenrec" > /dev/null || pgrep -f "gpu-screen-recorder" > /dev/null; then
    ...
    if [ -n "$FILE_PATH" ] && [ -f "$FILE_PATH" ]; then
        ...
        case "$ACTION" in
            default) xdg-open "$FILE_PATH" ;;
            folder)  xdg-open "$VIDEO_DIR" ;;
        esac
    fi
    exit 0
fi
...
case "$1" in
    --fullscreen-all)
        gpu-screen-recorder -w "$(getactivemonitor)" -f 60 -a "$(getsystemsound)|$(getmicinput)" -o "$FULL_PATH" &
        ;;
    --fullscreen-sound)
        gpu-screen-recorder -w "$(getactivemonitor)" -f 60 -a "$(getsystemsound)" -o "$FULL_PATH" &
        ;;
    --fullscreen)
        gpu-screen-recorder -w "$(getactivemonitor)" -f 60 -o "$FULL_PATH" &
        ;;
    --region|*)
        region=$(slurp) || {
        ...
        wl-screenrec -g "$region" -f "$FULL_PATH" &
        ;;
esac
disown